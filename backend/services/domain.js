...
export async function analyzeDomain(input) {
  ...
  if (foundTld) {
    result.score += 10;
    result.reasons.push(`Uses a suspicious TLD: ${foundTld}`);
  }
  ...
  if (domain.includes('uniswap') && !domain.endsWith('uniswap.org') && !domain.endsWith('uniswap.com')) {
    result.isSuspicious = true;
    result.score += 50;
    result.reasons.push('Possible Uniswap brand impersonation detected');
  }
  ...
  if ((domain.includes('claim') || domain.includes('airdrop')) && !domain.endsWith('.com') && !domain.endsWith('.org')) {
    result.isSuspicious = true;
    result.score += 30;
    result.reasons.push('Typical phishing domain pattern (claim/airdrop + non-standard TLD)');
  }
